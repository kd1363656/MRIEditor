...
void MRI::Editor::EditorHierarchyView::HandleHierarchyDragAndDrop(std::weak_ptr<MRI::GameObject> a_gameObjectCache)
{
	...
	const std::string& l_prefabName = std::string(l_parentCache->GetPrefabName());
	...
	// ヌルポインターでドロップ操作に失敗しているかを判別
	if (!l_dropped) { return; }
	
	// ドロップした対象が同じゲームオブジェクトなら"return"
	if (l_dropped == l_parentCache) 
	{
		MRI_ADD_LOG("ドロップ先のゲームオブジェクトが同一のゲームオブジェクトです、親子関係を構築できません。");
		return; 
	}
