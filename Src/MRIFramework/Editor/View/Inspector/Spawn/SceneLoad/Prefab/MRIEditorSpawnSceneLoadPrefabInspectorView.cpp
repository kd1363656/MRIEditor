...
void MRI::Editor::EditorSpawnSceneLoadPrefabInspectorView::DrawAddSceneLoadPrefabMapButton() const
{
	auto l_editorViewCache = MRI::Editor::EditorManager::GetInstance().FetchEditorViewCache<MRI::Editor::EditorPrefabGameObjectRegistryInspectorView>().lock();
	if (!l_editorViewCache) { return; }
	...
	MRI::EditorUtility::CreateGameObjectAndRegisterScenePrefab(m_addPrefabName , l_filePath);
}
void MRI::Editor::EditorSpawnSceneLoadPrefabInspectorView::DrawSceneLoadPrefabMap() const
{
	auto l_sceneCache = MRI::SceneManager::GetInstance().GetSceneCache().lock();
	if (!l_sceneCache) { return; }
