...
void MRI::Editor::EditorPrefabGameObjectRegistryInspectorView::Init()
{
	...
	m_saveSuccessPopup->SetPopupText(MRI::EditorCommonConstant::k_saveSuccessPopupText);
...
void MRI::Editor::EditorPrefabGameObjectRegistryInspectorView::LoadFile()
{
	...
	for (const auto& l_json : l_rootJson["AllPrefabMap"])
	{
		...
		const std::string l_prefabFilePath = l_json.value("PrefabFilePath" , MRI::CommonConstant::k_stringUnknown.data());
...
void MRI::Editor::EditorPrefabGameObjectRegistryInspectorView::DrawAddPrefabMapInfo()
{
	...
	auto l_filePath = std::string();

	// しっかり保存されていなければ追加しない
	if (!MRI::EditorUtility::SaveFileDialog(l_filePath										, 
											"ファイルを保存"							    , 
											MRI::EditorCommonConstant::k_jsonFilePathFilter ,
											"json"))
	{
		return;
	}
	...
	MRI::EditorUtility::CreateGameObjectAndRegisterScenePrefab(m_prefabNameBuffer , l_filePath);
}
