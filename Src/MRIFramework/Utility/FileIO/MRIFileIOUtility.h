...
namespace MRI::FileIOUtility
{
	...
	inline void SaveJsonFile(const nlohmann::json& a_json , const std::string& a_filePath)
	{
		...
		if (l_path.extension() != MRI::CommonConstant::k_jsonExtension)
		{
			l_path += MRI::CommonConstant::k_jsonExtension;
		}

		std::ofstream l_ofs(l_path , std::ios::out);

		l_ofs << a_json.dump(MRI::FileIOUtility::k_jsonIndentNum);
		l_ofs.close			();
	}
