...
bool Application::Init(const MRI::CommonStruct::Dimension2D& a_size)
{
	// タイトル名 + "FPS"の表示
	...
	// "TypeInfo"の登録
	// ===================================================================
	MRI::TypeInfoUtility::RegisterAll();
	
	// ===================================================================
	// "Factory"の登録
	// ===================================================================
	MRI::FactoryUtility::RegisterAll();

	// ===================================================================
	// "Editor"初期化
	// ===================================================================
	MRI::Editor::EditorManager::GetInstance().Init();

	// ===================================================================
	// シーンマネージャーの初期化、必要なファイルパスのセット
	// ===================================================================
	{
		auto& l_manager = MRI::SceneManager::GetInstance();

		l_manager.Init                     ();
		l_manager.SetSceneManagerFileIOPath(k_firstSceneFilePath);
		l_manager.LoadFile                 ();
		l_manager.PostLoadInit             ();
	}
...
{
	auto l_rootJson = MRI::FileIOUtility::LoadJsonFile(k_windowSizeFileIOPath);
...
void Application::SaveWindowSize()
{
	...
	MRI::FileIOUtility::SaveJsonFile(l_rootJson , k_windowSizeFileIOPath);
}

void Application::BeginUpdate() const
{
	// 空間環境の更新
...
{
	MRI::SceneManager::GetInstance().EarlyUpdate();
}
void Application::PreUpdate() const
{
	MRI::SceneManager::GetInstance().PreUpdate();
}
void Application::Update() const
{
	MRI::SceneManager::GetInstance().Update();
}
void Application::LateUpdate() const
{
	MRI::SceneManager::GetInstance().LateUpdate();
}
void Application::FixMatrix() const
{
	MRI::SceneManager::GetInstance().FixMatrix();
}
